...
type NoticeAudio string

const (
	Default        NoticeAudio = "ms-winsoundevent:Notification.Default"
...
type NoticeDuration string

const (
	Short NoticeDuration = "short"
	Long                 = "long"
)
...
//   - Title
//
// If no NoticeAudio is provided, then the toast notification will be silent.
...
// Audio returns a NoticeAudio given a user-provided input (useful for cli apps).
//
// If the "name" doesn't match, then the default NoticeAudio is returned, along with ErrorInvalidAudio.
//
...
func Audio(name string) (NoticeAudio, error) {
...
// Duration returns a NoticeDuration given a user-provided input (useful for cli apps).
//
// The default duration is short. If the "name" doesn't match, then the default NoticeDuration is returned,
...
//
// Handle the error appropriately according to how your app should work.
func Duration(name string) (NoticeDuration, error) {
	switch strings.ToLower(name) {
	case "short":
		return Short, nil
	case "long":
...
func invokeTemporaryScript(content string) error {
	id := uuid.New().String()
	file := filepath.Join(os.TempDir(), id+".ps1")
